...
def main(seed=2024):
    ...
    # split data
    Y_train = train_dataset.data.flatten(1).float()
    # Y_train -= Y_train.mean(); Y_train /= Y_train.std()
    ...
    latent_dim = 10
    n_inducing = 128
    n_class = len(torch.unique(labels_train))
    
    Y_test = test_dataset.data.flatten(1).float()
    # Y_test -= Y_train.mean(); Y_test /= Y_train.std()
    ...
    for k in torch.unique(labels_train):
        ...
        elbo_train = mll(model_train(sample[batch_idx]), Y_train_k[batch_idx].to(device).T).sum()
        # full model
        batch_test = np.random.default_rng(seed).choice(np.arange(n_test), size=test_size, replace=False)
        Y_full = torch.concatenate((Y_train_k, Y_test[batch_test])) # appending testing data
        nn_idx = []
        for yte in Y_test[batch_test]:
            nn_idx.append((Y_train_k-yte).norm(dim=-1).argmin())
        ...
        X_S_init = torch.nn.Parameter(torch.concatenate((model_train.X.q_log_sigma.detach(), torch.randn(test_size, latent_dim, device=device))))
        fix_param = {'model.X.q_mu': np.arange(Y_train_k.shape[0]),
                     'model.X.q_log_sigma': np.arange(Y_train_k.shape[0])}
        beginning=timeit.default_timer()
        ...
        # sample[:-test_size] = sample_
        batch_idx = np.concatenate((batch_idx, np.arange(Y_train_k.shape[0], Y_full.shape[0])))
        ...
        elbo_full = loglik[:,-test_size:].sum(0)+loglik[:,:-test_size].sum(0).mean()-kl.sum()
        
        # log prediction probabilities
        log_predprob[k.int()] = elbo_full - elbo_train + torch.log(labels_test[batch_test].eq(k).float().mean())
    # summarize losses
    import pandas as pd
    ...
    ACC = pred.eq(labels_test[batch_test].view_as(pred)).float().mean().cpu()
