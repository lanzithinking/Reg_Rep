...
loss_list = []
if os.path.exists(os.path.join('./results','dqep_q'+str(POWER.cpu().item())+'_'+str(model.num_layers)+'layers_mnist_checkpoint.dat')):
    state_dict = torch.load(os.path.join('./results','dqep_q'+str(POWER.cpu().item())+'_'+str(model.num_layers)+'layers_mnist_checkpoint.dat'), map_location=device)['model']
else:
    # set device
    ...
    torch.save({'model': state_dict}, os.path.join('./results','dqep_q'+str(POWER.cpu().item())+'_'+str(model.num_layers)+'layers_mnist_checkpoint.dat'))
...
# plt.tick_params(axis='both', which='major', labelsize=14)
# # plt.show()
# plt.savefig(os.path.join('./results','mnist_DQEP_q'+str(POWER.cpu().item())+'_'+str(model.num_layers)+'layers.png'),bbox_inches='tight')
...
plt.savefig(os.path.join('./results','mnist_DQEP_q'+str(POWER.cpu().item())+'_'+str(model.num_layers)+'layers_latentpairs.png'),bbox_inches='tight')