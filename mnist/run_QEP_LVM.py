...
POWER = torch.tensor(1.25, device=device)
...
if os.path.exists(os.path.join('./results','qeplvm_q'+str(POWER.cpu().item())+'_mnist_checkpoint.dat')):
    ...
    # state_dict, likelihood_state_dict = torch.load(os.path.join('./results','qeplvm_q'+str(POWER.cpu().item())+'_mnist_checkpoint.dat'), map_location=device).values()
else:
    # set device
