...
loss_list = []
if os.path.exists(os.path.join('./results','mnist_qeplvm_q'+str(POWER.cpu().item())+'_checkpoint.dat')):
    state_dict = torch.load(os.path.join('./results','mnist_qeplvm_q'+str(POWER.cpu().item())+'_checkpoint.dat'), map_location=device)['model']
    # state_dict, likelihood_state_dict = torch.load(os.path.join('./results','mnist_qeplvm_q'+str(POWER.cpu().item())+'_checkpoint.dat'), map_location=device).values()
else:
    # set device
    ...
    torch.save({'model': state_dict, 'likelihood': likelihood_state_dict}, os.path.join('./results','mnist_qeplvm_q'+str(POWER.cpu().item())+'_checkpoint.dat'))
...
plt.title('q = '+str(POWER.cpu().item())+(' (Gaussian)' if POWER.cpu().item()==2 else ''), fontsize=25)
plt.xlabel('Latent dim 1', fontsize=20)
plt.ylabel('Latent dim 2', fontsize=20)
plt.tick_params(axis='both', which='major', labelsize=15)
plt.savefig(os.path.join('./results','mnist_QEP-LVM_q'+str(POWER.cpu().item())+'_latent.png'),bbox_inches='tight')
...
# plt.savefig(os.path.join('./results','mnist_QEP-LVM_q'+str(POWER.cpu().item())+'_sampledigits.png'),bbox_inches='tight')