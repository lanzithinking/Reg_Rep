...
class bQEPLVM(BayesianQEPLVM):
    def __init__(self, n, data_dim, latent_dim, n_inducing, pca=False, power_init=torch.tensor(1.0)):
        ...
        self.power = Power(power_init, 
                           power_constraint=gpytorch.constraints.Interval(.01, 5.0),
                           power_prior=gpytorch.priors.GammaPrior(4.0, 2.0))
...
# plt.tick_params(axis='both', which='major', labelsize=14)
# # plt.show()
# plt.savefig(os.path.join('./results','./oilflow_QEP-LVM_q'+str(q)+'.png'),bbox_inches='tight')
#
...
#         # model.X.power = torch.tensor(q); model.X.prior_x.power = torch.tensor(q)
#         ps[i] = model.likelihood.log_marginal(Y.T, model(sample)).mean()
#         # ps[i] = model.likelihood.expected_log_prob(Y.T, model(sample)).mean()
#     ps = np.exp(ps-ps.max())
#     if normalize:
#         area = auc(qs, ps)
#         ps /= area
...
# plt.tick_params(axis='both', which='major', labelsize=14)
# plt.savefig(os.path.join('./results','./oilflow_densities_QEP-LVM_q'+str(q)+'.png'),bbox_inches='tight')