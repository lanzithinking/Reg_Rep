...
loss_list = []
if os.path.exists(os.path.join('./results','oilflow_gplvm_checkpoint.dat')):
    state_dict = torch.load(os.path.join('./results','oilflow_gplvm_checkpoint.dat'), map_location=device)['model']
else:
    # set device
    ...
    torch.save({'model': state_dict, 'likelihood': likelihood_state_dict}, os.path.join('./results','oilflow_gplvm_checkpoint.dat'))
