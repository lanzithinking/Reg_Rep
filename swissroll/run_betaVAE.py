"Beta Variatioanl AutoEncoder Model"
...
# Model
model = BetaVAE(data_dim, hidden_dim, latent_dim, loss_type='H')

# optimizer
optimizer = torch.optim.Adam(model.parameters(), lr=0.001)

loss_list = []
if os.path.exists(os.path.join('./results_'+dataset,dataset+'_betavae_checkpoint.dat')):
    state_dict = torch.load(os.path.join('./results_'+dataset,dataset+'_betavae_checkpoint.dat'), map_location=device)['model']
else:
    # set device
    ...
    for epoch in iterator:
        ...
        for x, _ in minibatch_iter:
            ...
            optimizer.zero_grad()
            output = model(x)
            loss = model.loss(*output, M_N=0.001)['loss']
            loss.backward()
            optimizer.step()
            loss_i += loss.item()
    ...
    torch.save({'model': state_dict}, os.path.join('./results_'+dataset,dataset+'_betavae_checkpoint.dat'))
...
X, log_vars = model.encode(Y.to(device))
...
# plt.show()
os.makedirs('./results_'+dataset, exist_ok=True)
plt.savefig(os.path.join('./results_'+dataset,dataset+'_betavae.png'),bbox_inches='tight')

fig = plt.figure(figsize=(7, 6))
plt.scatter(X[:, l1], X[:, l2], c=colors, alpha=0.8)
plt.title('Beta-VAE', fontsize=20)
# plt.axis('square')
plt.xlabel('Latent dim 1', fontsize=20)
plt.ylabel('Latent dim 2', fontsize=20)
plt.tick_params(axis='both', which='major', labelsize=15)
plt.savefig(os.path.join('./results_'+dataset,dataset+'_latent_betavae.png'),bbox_inches='tight')
...
plt.tick_params(axis='both', which='major', labelsize=15)
plt.savefig(os.path.join('./results_'+dataset,dataset+'_latdim_betavae.png'),bbox_inches='tight')
...
# plt.scatter(Z_[:, 0], Z_[:, 1], c=colors)
# plt.title('Beta-VAE', fontsize=20)
# plt.xlabel('Latent dim 1', fontsize=20)
# plt.ylabel('Latent dim 2', fontsize=20)
# plt.savefig(os.path.join('./results_'+dataset,dataset+'_betavae_latent.png'),bbox_inches='tight')