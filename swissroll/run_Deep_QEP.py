...
class DQEPLayer(DeepQEPLayer):
    def __init__(self, input_dims, output_dims, num_inducing=25, mean_type='zero', **kwargs):
        self.power = POWER
...
N, data_dim = Y.shape
latent_dim = data_dim
hidden_features = [data_dim, data_dim]
pca = False
...
plt.savefig(os.path.join('./results','swissroll_DQEP_'+str(model.num_layers)+'layers_q'+str(q)+'.png'),bbox_inches='tight')