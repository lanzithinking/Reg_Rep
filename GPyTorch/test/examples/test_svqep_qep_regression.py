...
class SVQEPRegressionModel(ApproximateQEP):
    ...
    def forward(self, x):
        ...
        latent_pred = gpytorch.distributions.MultivariateQExponential(mean_x, covar_x, power=self.power)
        return latent_pred
...
class TestSVQEPRegression(BaseTestCase, unittest.TestCase):
    ...
    def test_loading_old_model(self):
        ...
        model = SVQEPRegressionModel(torch.linspace(0, 1, 25), gpytorch.variational.CholeskyVariationalDistribution)
