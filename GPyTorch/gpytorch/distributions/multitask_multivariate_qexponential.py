...
class MultitaskMultivariateQExponential(MultivariateQExponential):
    ...
    def from_batch_qep(cls, batch_qep, task_dim=-1):
        ...
        :param ~gpytorch.distributions.MultivariateQExponential batch_qep: The base QEP distribution.
        ...
        :rtype: gpytorch.distributions.MultitaskMultivariateQExponential
        ...
        Example:
            ...
            >>> qep = gpytorch.distributions.MultivariateQExponential(mean, covar, power)
    ...
    def from_uncorrelated_qeps(cls, qeps):
        """
        Convert an iterable of QEPs into a :obj:`~gpytorch.distributions.MultitaskMultivariateQExponential`.
        The resulting distribution will have ``len(qeps)`` tasks, and the tasks will be uncorrelated.

        :param ~gpytorch.distributions.MultitaskQExponential qep: The base QEP distributions.
        ...
        Example:
            ...
            >>> covar = covar_factor @ covar_factor.transpose(-1, -2)
            >>> power = torch.tensor(1.0)
            >>> qep1 = gpytorch.distributions.MultivariateQExponential(mean, covar, power)
            >>>
            >>> mean = torch.randn(4, 3)
            >>> covar_factor = torch.randn(4, 3, 3)
            >>> covar = covar_factor @ covar_factor.transpose(-1, -2)
            >>> qep2 = gpytorch.distributions.MultivariateQExponential(mean, covar, power)
            >>>
        ...
        if not all(m.batch_shape == qeps[0].batch_shape for m in qeps[1:]):
            raise ValueError("All MultivariateQExponentials must have the same batch shape")
    ...
    def from_repeated_qep(cls, qep, num_tasks):
        """
        Convert a single QEP into a :obj:`~gpytorch.distributions.MultitaskMultivariateQExponential`,
        where each task shares the same mean and covariance.

        :param ~gpytorch.distributions.MultitaskQExponential qep: The base QEP distribution.
        ...
        Example:
            ...
            >>> covar_factor = torch.randn(4, 3, 3)
            >>> covar = covar_factor @ covar_factor.transpose(-1, -2)
            >>> qep = gpytorch.distributions.MultivariateQExponential(mean, covar)
            >>> print(qep.event_shape, qep.batch_shape)
    ...
    def to_data_uncorrelated_dist(self, jitter_val=1e-4):
        ...
        :rtype: gpytorch.distributions.MultivariateQExponential
        """
